...
func main() {
	...
	// Collect filenames of posts to process.
	...
	if !fi.IsDir() {
		...
		filenames = []string{filepath.Base(cfg.Source)}
		cfg.Source = filepath.Dir(cfg.Source)
	} else {
		// User specified a directory. Recursively collect all json files.
